...
# 定义 PointAttN 模型中的 PCT_refine 类
...
#         # lst1 和 lst2 的声明 ——
#         self.lst1 = LightSpatialTransformerLayer(
#             d_model=512,  # 与 cross_transformer 输出通道保持一致
#             base_nsample=16,  # 你希望每个 query box 采样多少点
#             use_xyz=False  # 如果你想在 attention 中加入坐标信息
#         )
#         self.lst2 = LightSpatialTransformerLayer(
#             d_model=512,
#             base_nsample=16,
#             use_xyz=False
#         )
#         # 三个 cross_transformer 模块，用于特征交互
...
#         # 将 y 和扩展后的 feat_g 拼接在一起
#         y0 = torch.cat([y, feat_g.repeat(1, 1, y.shape[-1])], dim=1)
#         # 通过三个自注意力模块逐层细化特征
#         # 第一个 cross attention
#         y1 = self.sa1(y0, y0)  # 输入 [B, 256, N]，输出 [B, 512, N]
#         # 插入 LST1：输入坐标和 feature
#         y1 = self.lst1(xyz, y1)  # 注意 LST 输出仍为 [B, C, N]，C = 512
#         # 第二个 cross attention
#         y2 = self.sa2(y1, y1)
#         # 插入 LST2
#         y2 = self.lst2(xyz, y2)
#         # 第三个 cross attention，输出扩展通道数
#         y3 = self.sa3(y2, y2)  # 输出 [B, channel * ratio, N]
#         # 对细化后的特征进行卷积并调整形状
